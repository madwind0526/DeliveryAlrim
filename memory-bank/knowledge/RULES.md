...
**규칙:** 배송 수동 등록은 디버그/테스트 화면에 두지 않고 홈 배송 목록의 `+` FAB에서 제공한다. 화면 제목과 라우트도 디버그 문구를 쓰지 않는다.
**이유:** 자동 수집이 놓친 배송을 사용자가 직접 보완하는 핵심 사용 흐름이다. 디버그 기능처럼 보이면 실사용자가 찾기 어렵고 테스트 기능으로 오해할 수 있다.
**적용 시점:** 배송 목록, 라우팅, 설정 테스트 섹션, 수동 등록 화면을 수정할 때.
