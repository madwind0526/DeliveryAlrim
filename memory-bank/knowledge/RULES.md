...
## UI 수정은 기존 정상 동작 회귀 검증 필수

**규칙:** UI 버그를 고칠 때는 기존에 정상 동작하던 진입 플로우를 먼저 확인하고, 수정 후 같은 플로우를 다시 검증한다. 다이얼로그 수정은 최소한 `버튼 탭 → 팝업 표시 → 필드 포커스 → 키보드 표시 → 저장/취소`를 확인한다.
**이유:** overflow를 고치기 위해 `AlertDialog` 구조를 바꾸다가 팝업 내용이 보이지 않고 barrier만 뜨는 회귀가 발생했다. 목표 오류만 보는 검증은 기존 정상 동작을 깨뜨릴 수 있다.
**적용 시점:** Flutter 화면, 다이얼로그, 네비게이션, 저장/취소 버튼, 반응형 UI를 수정할 때 항상.
