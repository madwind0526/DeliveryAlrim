...
- **Status:** In Progress
- **Cache Status:** CLEAN
- **Last Checkpoint:** 2026-07-06 code-review.md 지적사항 추가 반영, 카카오 캡처 단일 저장 경로 정리, debug/release 빌드와 실기기 업데이트 검증
...
- `code-review.md` 기준 수정된 항목: secure storage 실패 방어, 수동 등록 운송장 패턴 검증, 카카오 최신 캡처 clear, 릴리즈 빌드 테스트 주입 UI 숨김, 네이티브/Drift 동일 SQLite 동시 쓰기 제거, deliveredAt 보존, 접근성 root recycle, 하단 네비게이션 접근성 label 복구.
- User 소스 on/off 상태는 secure storage에 저장하며, 카카오톡 자동 동기화는 카카오톡 on/off 상태를 따른다.
- 남은 구조 이슈: NotificationListener 실제 파서 구현, Gmail/IMAP 실제 연결, Android SMS 권한/수신 연결, 카카오톡 계정 적용 범위 정리, 배터리 최적화 대응.
