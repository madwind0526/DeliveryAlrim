...
| 규칙/원칙 | 카드결제로 등록된 주문(registered, 항상 non-terminal)은 buildParcelDayIndex의 carry-forward 규칙상 오늘까지 매일 캘린더에 계속 표시된다 — 자동 완료/만료 시점이 없어 캘린더가 점점 붐빌 수 있음(2026-07-12 설계 트레이드오프, 아직 미해결). 사용자가 필요시 수동 삭제하거나, 추후 "주문 확인 완료" 액션을 추가하는 방안 검토 | `knowledge/RULES.md` |
