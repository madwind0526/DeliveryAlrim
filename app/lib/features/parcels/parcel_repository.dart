...
class LocalParcelRepository implements ParcelRepository {
  ...
  Stream<List<Parcel>> _watchWhere({required bool? done}) {
    ...
    if (done != null) {
      // Card/mall order placeholders are a point-in-time payment/order
      // record, not a shipment — they never belong in the active or done
      // shipment lists, only on their day in the calendar
      // (parcel_day_index.dart).
      final placeholderCodes = [Couriers.cardOrder.code, Couriers.mallOrder.code];
      query.where(
        (t) =>
            t.courierCode.isNotIn(placeholderCodes) &
            (done
